...
app.get("/", async (req, res) => {
  try {
    ...
    const [contactEmplpyRows]= await db.query("SELECT*FROM contact_employment ")
    const[documentsRows]=await db.query("SELECT*FROM documents  ")
    // Send both as a single JSON response
    res.json({
      otps: otpRows,
      personalInfo: personalRows,
      addressInfo:addressRows,
      contactEmploymentInfo:contactEmplpyRows,
